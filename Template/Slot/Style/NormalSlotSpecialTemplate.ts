 ...
 * @Version 1.1
 */
export default class NormalSlotSpecialTemplate extends ASlotTemplate {
    ...
    constructor(styleData: fcc.SlotImgSpecialSetting, configManager: fcc.IF.ISlotConfigManager) {
        ...
        this.lookAtTime = styleData.lookAtTime;                                         //拿取瞇排時間
    ...
    protected executeSlotAnimation(index: number, resolve?: () => void, numberOfTimes: number = 0): void {
        ...
        const duration: number = this.slotGirdSpeed / this.speedRatio;
        /*拿取該列,目的:對該列執行LOOP事件*/
            ...
            .call(() => {
                ...
                if (this.isResultOK) {
                    ...
                    if (index == 0 && numberOfTimes == rowEndCount) {
                        ...
                        //如果當前是第一列,已達到需轉動的次數
                        this.showAnswerAnimation(index);
                        ...
                        this.isSlotEnd[index] = true;
                        //執行顯示結果動畫
                        this.showAnswerAnimation(index, resolve);
     ...
     * @param resolve
     */
    protected showAnswerAnimation(index: number, resolve?) {
        //更新正確答案 答案更新再每列最上面位置
        this.updateGridAnswer(index)
        //緩動時間 = 當前一個格子的跑速 * 有幾個格子 * 當前是否加速(無加速 = 1)
        const duration = this.slotGirdSpeed * this.slotRowGridCount / this.speedRatio;
